...
test('processing mail transaction, concurrency, review and replay against isolated PostgreSQL',{skip:!url},async t=>{
 ...
 const seed=async(status='paid')=>{
  await pool.query(`INSERT INTO account_requests(id,school_name,emails,type,account_type,quantity,extension_date,status,confirmed_at,processing_email_sent_at,processing_email_send_started_at,partner_lifecycle_state,market_void_state,channel,notes,created_at,updated_at)
    VALUES (222,'Example High','one@example.test, two@example.test','upgrade','teacher',2,null,$1,null,'2026-09-08',null,'active','active','school_store',null,now(),now()),(223,'Example Middle','other@example.test','extension','teacher',1,'2027-09-11','paid',null,'2026-09-08',null,'active','active','school_store',null,now(),now())
    ON CONFLICT(id) DO UPDATE SET status=excluded.status,confirmed_at=null,market_void_state='active',processing_email_sent_at='2026-09-08',partner_lifecycle_state='active',quantity=excluded.quantity,updated_at=now()`,[status]);
 };
