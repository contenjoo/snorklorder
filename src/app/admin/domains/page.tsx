...
import { PageHeader, EmptyState, Spinner, Card } from "@/components/admin/ui";
...
const STATUSES = [
  { value: "pending", label: "Jon 처리대기", color: STATUS_CHIP.pending },
  { value: "done", label: "활성화 완료", color: STATUS_CHIP.done },
  { value: "invoiced", label: "인보이스 발행", color: STATUS_CHIP.invoiced },
...
export default function DomainsPage() {
  ...
  return (
    <div className="space-y-4 pb-20 md:pb-0">
      <PageHeader
        title="도메인 유료 등록"
        actions={msg ? <span className="text-xs text-emerald-600 font-medium">{msg}</span> : undefined}
      >
        <span><strong className="text-slate-900 text-sm">{rows.length}</strong> 건</span>
      </PageHeader>

      {rows.length === 0 ? (
        <Card>
          <EmptyState icon="🌐" title="아직 도메인 요청이 없습니다"
            hint='학교 관리에서 도메인 입력 후 "Jon에게 유료 등록 요청" 버튼으로 생성하세요.' />
        </Card>
      ) : (
            ...
            return (
              <div key={r.id} className="px-4 py-3">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-sm text-slate-700">{r.schoolNameEn || r.schoolName}</span>
                  {r.team && <span className="text-[10px] text-slate-400">{r.team}</span>}
                  <select
                    value={r.status}
                    onChange={(e) => updateRow(r.id, { status: e.target.value })}
                    className={`ml-auto text-[10px] font-medium rounded-full px-2 py-0.5 border-0 cursor-pointer ${st?.color || "bg-slate-100"}`}
                  >
                    {STATUSES.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
                  </select>
                  <button onClick={() => setEditing(editing?.id === r.id ? null : r)} className="text-xs text-slate-400 hover:text-slate-700">
                    {editing?.id === r.id ? "닫기" : "💳 인보이스"}
                  </button>
                  <button onClick={() => deleteRow(r.id)} className="text-xs text-slate-300 hover:text-red-500">✕</button>
                </div>
                ...
                {editing?.id === r.id && (
                  <div className="mt-3 grid grid-cols-2 gap-2 bg-slate-50 rounded-lg p-3">
